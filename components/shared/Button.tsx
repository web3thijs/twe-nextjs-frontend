...
const Button = ({text, outline, className, callback}: ButtonProps) => {
    if(outline == false){
        ...
        return <button type="button" onClick={callback} className="text-base text-center text-greenish font-semibold bg-white shadow-3xl rounded-2xl px-8 py-3 border-solid border-2 border-grey-light">{ text }</button>
    }
}
